...
- credentials or authentication: `go-authentication`;
- access decisions: `go-authorization`;
- rate algorithms and distributed quotas: `go-rate-limit`;
- idempotency records or operation semantics: `go-idempotency`;
- tracing SDK/exporter lifecycle: `go-telemetry`;
- logging backends: `go-log` and `log/slog`;
- routing: `go-router`;
- server/process lifecycle: `go-service`;
- response caching stores or refresh algorithms: `go-cache` and application
  policy;
- circuit-breaker state: `go-circuit-breaker`.
...
- Resolve ownership overlap with `go-service`: one implementation MUST become
...
- Resolve ownership overlap with `go-service` and prevent duplicate recovery
...
- Integrate with `go-clock` only where deterministic time control warrants it.
...
- `go-log` and `go-telemetry` adapters consume events; core has no exporter,
...
- `go-wire`, `go-jsonapi`, `go-jsonrpc`, and applications own representation
...
- Health and readiness handlers remain owned by `go-service`.
...
- integration chains with `go-router`, `go-service`, `go-authentication`,
  `go-authorization`, `go-rate-limit`, `go-idempotency`, and `go-telemetry`;
- tests proving duplicate installation with `go-service` cannot occur silently.
