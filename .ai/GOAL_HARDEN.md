...
Prove that `go-http-middleware` is standards-aligned, deterministic, bounded,
...
- Verify `go-log` and `go-telemetry` adapters do not create exporters, duplicate
...
- Reconcile request ID and recovery ownership with `go-service`; one
...
- Test representative chains with `go-router`, JSON-RPC, webhooks, health
