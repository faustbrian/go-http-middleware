...
The sibling contract was audited on 2026-07-18 against `go-service` remote
...
current `go-service` default server, omit local recovery, request ID, and body
...
|---|---|---|
| server lifecycle, listener, shutdown, default core middleware | `go-service` | detect named overlap only |
| routes, parameters, route-local middleware | `go-router` | wrap compiled handler and observe bounded route name |
| authentication and credentials | `go-authentication` | adapt standard middleware by name |
| authorization decisions | `go-authorization` | adapt standard middleware by name |
| quotas and distributed rate state | `go-rate-limit` | local admission remains concurrency-only |
| replay and idempotency state | `go-idempotency` | preserve owning handler contract |
| spans, exporters, propagators, and sampling | `go-telemetry` | consume a bounded completion event only |
| logging backend and sinks | `go-log` | consume a bounded completion event only |

The `go-router` contract was audited at remote commit
