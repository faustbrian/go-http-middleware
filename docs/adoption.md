...
Use recovery, observation, trusted proxy policy, request IDs, CORS, API security
...
delegate replay/idempotency state to `go-idempotency`. CORS is normally absent
...
`go-service` owns health and readiness handlers. A transport-only state source
