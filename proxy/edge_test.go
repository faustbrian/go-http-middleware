...
func TestPolicyAndDirectInformationBoundaries(t *testing.T) {
	t.Parallel()
	for _, policy := range []Policy{{Mode: XForwarded + 1}, {MaxHops: -1}, {MaxHops: 129}, {MaxHeaderBytes: -1}, {MaxHeaderBytes: 1<<20 + 1}} {
		_, err := New(policy)
...
func TestRFCForwardedMalformedMatrix(t *testing.T) {
	...
	direct := Info{ClientIP: netip.MustParseAddr("10.0.0.3"), Host: "internal", Scheme: "http", Provenance: Direct}
	trusted := []netip.Prefix{netip.MustParsePrefix("10.0.0.0/8")}
	for _, value := range []string{"", "for=198.51.100.1;", "for=198.51.100.1,for=10.0.0.2,for=10.0.0.1", "broken", "for=", "for=_hidden", "proto=https", "for=198.51.100.1;proto=ftp", "for=198.51.100.1;host=bad/path"} {
