...
.PHONY: actionlint api-compat api-update benchmark check check-all coverage docs format \
	format-check fuzz integration leak lint mutation nilaway race staticcheck \
	standards test tidy-check vet vuln
...
check: tidy-check format-check vet test race coverage fuzz mutation leak \
	integration standards benchmark docs api-compat actionlint lint staticcheck vuln
